...
export function BillingList({ paymentGroupsByDate }: BillingListProps) {
  ...
  const getStatusBadge = (status: string | null) => {
    switch (status) {
      case 'paid':
        return <Badge className="bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400">Fizetve</Badge>;
      case 'overdue':
        return <Badge className="bg-red-100 text-red-800 dark:text-red-400">Késedelmes</Badge>;
      case 'cancelled':
        return <Badge className="bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200">Törölve</Badge>;
      default:
        return <Badge className="bg-yellow-100 text-yellow-800 dark:text-yellow-400">Függőben</Badge>;
    }
  };
      ...
      {sortedDates.length === 0 ? (
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">Nincsenek esedékes számlák</h3>
            <p className="text-gray-500 dark:text-gray-400">Jelenleg nincs kiállítandó számla.</p>
          </CardContent>
        </Card>
      ) : (
        sortedDates.map((date) => {
          ...
          return (
            <Card key={date} className={`overflow-hidden ${isDateOverdue ? 'border-red-300 dark:border-red-800' : ''}`}>
              <CardHeader
                className={`cursor-pointer transition-colors ${
                  isDateOverdue ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-800'
              ...
              >
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <div>
                      <CardTitle className={`text-xl font-bold ${isDateOverdue ? 'text-red-700 dark:text-red-400' : 'text-gray-900 dark:text-gray-100'}`}>
                        {formatDate(date)}
                        {isDateOverdue && (
                  ...
                  <div className="divide-y divide-gray-200 dark:divide-gray-700">
                    {paymentGroups.map((pg) => {
                      ...
                      return (
                        <div
                          key={pg.id}
                          className={`p-6 ${itemOverdue ? 'bg-red-50 dark:bg-red-900/20' : 'hover:bg-gray-50 dark:hover:bg-gray-800'} transition-colors`}
                        >
                          <div className="flex justify-between items-start gap-6">
                            ...
                            <div className="flex-1 space-y-4">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="space-y-2">
                                  <div className="flex items-center gap-2 text-sm">
                                    <Banknote className="h-4 w-4 text-gray-400 dark:text-gray-500" />
                              ...
                              <div className="flex items-center gap-4 pt-2 border-t border-gray-100 dark:border-gray-800">
                                <div className="flex items-center gap-1.5 text-sm">
                                  <div className={`w-2 h-2 rounded-full ${pg.billCreated ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}`} />
                                  <span className={pg.billCreated ? 'text-green-700 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}>
                                    Számla létrehozva
                                  </span>
                                  {pg.billCreatedAt && (
                                ...
                                <div className="flex items-center gap-1.5 text-sm">
                                  <div className={`w-2 h-2 rounded-full ${pg.billSent ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}`} />
                                  <span className={pg.billSent ? 'text-green-700 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}>
                                    Számla elküldve
                                  </span>
                                  {pg.billSentAt && (
                                    <span className="text-xs text-gray-400 dark:text-gray-500">
                                ...
                                <div className="flex items-center gap-1.5 text-sm">
                                  <div className={`w-2 h-2 rounded-full ${pg.status === 'paid' ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}`} />
                                  <span className={pg.status === 'paid' ? 'text-green-700 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}>
                                    Fizetve
                                  </span>
                                  {pg.paidAt && (
                                    <span className="text-xs text-gray-400 dark:text-gray-500">
                              ...
                              {!pg.billCreated && (
                                <div className="flex items-center gap-2 pt-2">
                                  <input
                                    ...
                                    className="px-3 py-1.5 text-sm border border-gray-200 dark:border-gray-600 rounded-md w-48 dark:bg-gray-800 dark:text-gray-100"
                                  ...
                                  <input
                                    ...
                                    className="px-3 py-1.5 text-sm border border-gray-200 dark:border-gray-600 rounded-md flex-1 dark:bg-gray-800 dark:text-gray-100"
                            ...
                            <div className="flex flex-col gap-4 min-w-[180px] pl-4 border-l border-gray-200 dark:border-gray-700">
                              {!pg.billCreated && (
                                <Button
                                  type="button"
                                  size="default"
                              ...
                              {pg.status === 'paid' && (
                                <div className="flex items-center justify-center gap-2 text-green-600 dark:text-green-400 py-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                              ...
                              {pg.status !== 'paid' && pg.billSent && (
                                <div className="flex items-center justify-center gap-2 text-amber-600 dark:text-amber-400 py-2 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
