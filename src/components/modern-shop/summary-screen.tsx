...
const SummaryScreen: React.FC<SummaryScreenProps> = ({ orderState, updateOrder, onBack, onSubmit, isSubmitting = false, submitError = null }) => {
  ...
  return (
    <main className="container mx-auto px-6 py-12 max-w-4xl text-left animate-fade-in flex-grow text-balance">
      ...
      <h2 className="text-3xl font-black text-gray-900 dark:text-gray-100 tracking-tight uppercase mb-10 text-left">Összesítés és fizetés</h2>
      ...
      <div className="bg-white dark:bg-gray-900 p-6 md:p-8 rounded-[2.5rem] border border-gray-200 dark:border-gray-700 shadow-sm mb-10 text-left relative overflow-visible">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] text-left">Választott szállítási napok</h3>
          
          <div className="group relative z-20">
            <button className="flex items-center gap-2 text-gray-400 dark:text-gray-500 hover:text-emerald-600 transition-colors py-1">
            ...
            <div className="absolute right-0 top-full mt-2 w-72 bg-gray-900 text-white p-5 rounded-2xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform origin-top-right group-hover:translate-y-0 translate-y-2 pointer-events-none group-hover:pointer-events-auto">
              <h4 className="font-bold text-xs uppercase tracking-wider mb-3 text-gray-400 dark:text-gray-500">Szállítási díj alkalmanként</h4>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between items-center">
                  <span className="text-gray-300 dark:text-gray-600">10-25 palack között</span>
                  <span className="font-bold">{formatCurrency(CONSTANTS.SHIPPING_FEE_HIGH)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-300 dark:text-gray-600">26-49 palack között</span>
        ...
        <div className="space-y-3">
          {Object.entries(groupedDeliveriesByMonth).map(([monthKey, monthData]) => {
            ...
            return (
              <div key={monthKey} className="border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="text-lg font-bold text-gray-900 dark:text-gray-100">{monthData.month}</span>
                    <span className="px-2 py-0.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md text-xs font-bold text-gray-500 dark:text-gray-400">
                      {monthData.year}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                ...
                {isExpanded && (
                  <div className="p-4 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 space-y-3 animate-fade-in">
                    {monthData.dates.map((d, i) => (
                      <div key={i} className="grid grid-cols-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
                        {/* Left Column: Date */}
                        ...
                        <div className="text-right flex flex-col items-end justify-center">
                          <div className="flex items-baseline gap-1">
                            ...
                            <span className="text-sm font-black text-gray-900 dark:text-gray-100">{formatCurrency(itemTotal)}</span>
                          </div>
                          ...
                          {shippingFee > 0 ? (
                            discountApplied ? (
                              ...
                              <span className="text-[10px] text-gray-400 dark:text-gray-500">+ {formatCurrency(shippingFee)} szállítás</span>
                            )
                          ) : (
         ...
         {appliedCoupon ? (
             ...
             <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-800">
               <div className="flex items-center justify-between mb-4">
                 <h3 className="text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-[0.2em]">Partnerkód megadása</h3>
                 <button
                   type="button"
               ...
               <div className="flex flex-col sm:flex-row gap-3">
                  <input 
                      ...
                      className="flex-grow p-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-emerald-100 transition-all text-gray-700 dark:text-gray-300 placeholder:text-gray-400 dark:placeholder:text-gray-500"
                  />
                  <button 
                      type="button"
             ...
             <button
               ...
               className="w-full flex items-center justify-center gap-3 py-4 px-6 rounded-2xl transition-all text-emerald-700 font-bold text-sm uppercase tracking-wider hover:bg-emerald-50 dark:hover:bg-emerald-900/20 group"
             >
      ...
      {/* COST SUMMARY */}
      <div className="bg-white dark:bg-gray-900 p-6 md:p-8 rounded-[2.5rem] border border-gray-200 dark:border-gray-700 shadow-sm mb-10">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-baseline gap-3">
              <span className="font-black text-gray-900 dark:text-gray-100 uppercase text-sm">Rostik</span>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-baseline gap-3">
              <span className="font-black text-gray-900 dark:text-gray-100 uppercase text-sm">Szállítás</span>
              <span className="text-sm text-gray-500 dark:text-gray-400">
                {shippingFee > 0 ? (
                  <>
          ...
          <div className="border-t border-gray-100 dark:border-gray-800 pt-4 flex items-center justify-between">
            <span className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">Fizetendő</span>
